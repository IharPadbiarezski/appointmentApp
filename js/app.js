...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayAppointments() {
    ...
    let objectStore = DB.transaction("appointments").objectStore(
      "appointments"
    );
    ...
    objectStore.openCursor().onsuccess = function(e) {
      ...
      if (cursor) {
        ...
        appointmentHTML.innerHTML = `  
                         ...
                         <p class="font-weight-bold">Owner Name:  <span class="font-weight-normal">${
                           cursor.value.ownername
                         }<span></p>
                         <p class="font-weight-bold">Phone:  <span class="font-weight-normal">${
                           cursor.value.phone
                         }<span></p>
                         <p class="font-weight-bold">Date:  <span class="font-weight-normal">${
                           cursor.value.date
                         }<span></p>
                         <p class="font-weight-bold">Time:  <span class="font-weight-normal">${
                           cursor.value.hour
                         }<span></p>
  ...
  function removeAppointment(e) {
    ...
    let appointmentID = Number(
      e.target.parentElement.getAttribute("data-appointment-id")
    );
    // use a transaction
    let transaction = DB.transaction(["appointments"], "readwrite");
    let objectStore = transaction.objectStore("appointments");
